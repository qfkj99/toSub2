...
浏览器打开 `http://服务器IP:4399`，本机部署也可访问 `http://127.0.0.1:4399`。镜像包含 Node.js 22、Python 和 `curl_cffi`，以非 root 用户运行；Compose 负责进程回收、异常重启和数据卷挂载，无需额外运行 PM2。容器关闭前有 30 秒用于保存任务状态。容器内关闭前端热更新，不需要映射额外的 WebSocket 端口。

默认绑定宿主机所有 IPv4 地址（`0.0.0.0:4399`），可通过服务器 IP 访问。宝塔部署需要在宝塔防火墙和云服务器安全组放行 TCP 4399，建议将来源限制为自己的公网 IP。需要修改端口时，在项目根目录新建 `.env`，填写：
